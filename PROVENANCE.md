...
7. **Internal version label MUST match the tag.** Four sources of truth must align (bump them together when cutting a release):
   ...
   - **Gate 3 — `oci-publish.yml` boot+version gate, BEFORE attestation.** The `gate` job runs first in `oci-publish.yml`, as a matrix over **both arches on their native runners** (amd64 → `ubuntu-22.04`, arm64 → `ubuntu-24.04-arm`): each boots a clean `postgres:18-trixie`, installs the exact same-arch release-tarball bytes the leaf job is about to push, runs `CREATE EXTENSION pgrdf VERSION '<TAG>'`, and asserts `pg_extension.extversion == TAG` and `pgrdf.version() == TAG`. The leaf publish+attest job declares `needs: gate`, which waits on **every** matrix leg, so **no SLSA attestation is ever issued unless the `.so` actually loaded on the contractual base on both arches and reported the matching version** — attestation is gated on a working, version-correct install, not merely on a pushed digest. (Boot-testing amd64 only is precisely how v0.6.20's non-loading arm64 leaf shipped attested — issue #67; the matrix closes that hole.) A second, post-publish consumer-style smoke (pull the published artifact via ORAS, boot `postgres:18-trixie`, re-run the same `CREATE EXTENSION`) then runs in the index job before `update-latest-md.yml` is triggered — defence-in-depth on the round-tripped OCI bytes. Either gate failing is fail-fast: nothing downstream fires, the GitHub release stays a draft (Rule 8), and `LATEST.md` stays at the prior version. A wrong-labeled tag exists at most as an orphan GHCR digest — never attested-and-advertised. (`postgres:18-trixie` is the contractual base — see Rule 9.)
...
8. **Attestation is gated on a live boot+version check, and a code release is never advertised without a verified OCI bundle.** Two coupled invariants:
   ...
   - **Attest only past the gateway.** `oci-publish.yml`'s `gate` job boots a clean `postgres:18-trixie` on **each arch's native runner**, installs the exact same-arch release-tarball bytes about to be pushed, runs `CREATE EXTENSION pgrdf VERSION '<TAG>'`, and asserts both `pg_extension.extversion` and `pgrdf.version()` equal `<TAG>`. The publish+attest job declares `needs: gate`, so **no SLSA attestation is ever issued unless a container started and reported the matching version on both arches** (Gate 3 above). A pushed digest is not enough — a *working, version-correct install* is the precondition for attestation.
...
9. **Base image / ABI: the published `.so` targets Debian `trixie` on PostgreSQL 18, and pgRDF owns that choice for the whole downstream bundle chain.** The release binary is built for and validated against `postgres:18-trixie` (glibc); the artifact name encodes the PG major (`pgrdf-<ver>-pg18-glibc-<arch>.tar.gz`), and every gate (`ci.yml`, `compose/compose.yml`, the `oci-publish` `gate` + post-publish smoke) boots exactly that image. Per the standing agreement with downstream consumers (`pgrdf-owns-pg-base-release`): pgRDF's compile target dictates the Debian release the entire `pg_base` → bundle chain inherits — consumers must not choose it independently. As of ck-allinone **v0.7.31** the downstream bundle chain is **trixie-only**; there is no bookworm consumer path.

   **Both arches are built INSIDE the `postgres:18`/`trixie` builder container (`compose/builder.Containerfile`, `rust:1.96-trixie`), each on its native runner** — never bare on the runner OS, never cross-compiled. The container userland, not the runner, fixes the glibc floor, so the amd64 and arm64 leaves floor *consistently* from the *same commit*. This is the invariant issue #67 restored: v0.6.20 built bare on the runner, which floored amd64 at glibc 2.34 (`ubuntu-22.04`) and arm64 at 2.39 (`ubuntu-24.04-arm`) — an accident of runner selection that shipped a 2.39-floored arm64 `.so` unable to load on the base, attested because the gate only booted amd64. From **v0.6.21** the floor is a decision, not an accident, and it is *measured* out of the shipped `.so` (`objdump -T`) into `MANIFEST.json` (`platform.glibc_floor`) rather than hand-asserted; the bundle also carries `org.opencontainers.image.base.name`, `org.opencontainers.image.revision`, and `io.styk.pgrdf.build-base` annotations so a consumer verifies the base + commit as *facts*.

   **A base-release move (e.g. `bookworm` → `trixie`) is SONAME-breaking** — a `.so` linked against one Debian release fails to load (`undefined symbol` at `CREATE EXTENSION`, or `libicuuc.so.<N>: cannot open` at `initdb`) on base PostgreSQL binaries from another. So any base-release move MUST (a) bump the validation image in **all** gates in lockstep (`ci.yml`, `compose/compose.yml`, `oci-publish` gate + smoke, and `compose/builder.Containerfile`), and (b) coordinate with downstream consumers *before* the tag so their `pg_base` can regenerate its lib closure ahead of time. Coordination is **board-driven** on the styk-tv Application Planning Division project (public repos: an issue proposes, a PR `Closes` it, the thread is the record); a private `_WIP/NOTIFIES.*` is used only when the destination repo is private. The `bookworm` → `trixie` move itself was executed in v0.6.21 under issue #67, gated by the dual-arch boot check above.

10. **Every bundle ships a self-describing `MANIFEST.json`.** Each release tarball carries a `MANIFEST.json` (next to `SHA256SUMS`) declaring `version`, `extversion`, `runtime` (`pgrdf.version()` value + that it is `env!(CARGO_PKG_VERSION)`, compile-time), the install/upgrade SQL filenames, platform (`pg_major`/`arch`/`base`/`build_base`/`glibc_floor` — the last *measured* from the shipped `.so`, see Rule 9), and build (`tag`/`git_sha`/`built`). It is generated in `release.yml`'s repack step **from the actual repacked artifact** (control `default_version` + SQL filenames read back), so it cannot disagree with what shipped; the post-build step asserts `version == extversion == runtime.value == tag`, and the `oci-publish` gate boots the published `.so` and cross-checks `MANIFEST.json`'s `runtime.value` against the *actual* `pgrdf.version()` (validate the manifest against the artifact, not the source). This is the one place a downstream consumer reads a component's version — the gap that let a consumer pin (oci-germination `versions.yaml`) rot stale. `tools/validate-bundle.sh` is the cheap static gate a consumer runs on a pulled artifact. (Rule 7 keeps the *labels* honest; Rule 10 makes them *self-describing + machine-checkable*.)
