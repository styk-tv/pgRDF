...
- ✅ **G3 (§5 + §6) — v0.5.0-rc1 cut.** `pgrdf.validate(data,
  ...
  into CI on every PG major — Core **24/24 full-pass** on the
  `sh:conforms` invariant (ERRATA.v0.5 E-013); one W3C Core
  fixture (`prop-nodeKind-001`) documented-excluded for an
  upstream `sh:nodeKind` bug (E-013, a Phase H+I follow-up).
  ...
  (270 → 274). Closes v0.5-FUTURE **§5 + §6 — all v0.5-gate
  tracks §3-§8 complete**; released as **v0.5.0-rc1** (a release
  *candidate* — final v0.5.0 follows after Phase H+I hygiene +
  the two documented E-012/E-013 follow-ups).

**Phase G complete; the v0.5-FUTURE v0.5-gate scope is COMPLETE
(§3-§8 all shipped).** Next: **Phase H+I** — final hygiene +
ERRATA close-outs + the `executor.rs` core-BGP carve catch-up +
the E-012/E-013 SHACL follow-ups + the final **v0.5.0** cut+tag
(one release event; Phase H+I merged per the release-cadence
directive). Not started in G3.
