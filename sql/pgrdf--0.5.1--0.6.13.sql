-- pgrdf--0.5.1--0.6.13.sql
--
-- Upgrade-path declaration from v0.5.1 (the earliest installable version) to v0.6.13. PostgreSQL
-- requires this file to exist for `ALTER EXTENSION pgrdf UPDATE TO '0.6.13'` to be a valid path.
--
...
-- -> 0.6.13 path carries that DDL. v0.6.11 (R2.1) adds only the `load_turtle_staged_run` coordinator
-- FUNCTION (+ the CALL-able `load_turtle_staged` wrapper), v0.6.12 only corrects that loader's
-- literal-dictionary keying, and v0.6.13 only hardens the staged worker's panic-reporting + RESOLVE
-- memory — all ship in the base `.so` SQL, no new schema. The DDL this upgrade
...
--   or the v0.6.13 `.so` (whose bulk-rebuild references `lexical_md5`) breaks against the old schema.
...
--
-- The authoritative full surface ships in the base install script `pgrdf--0.6.13.sql`, which a fresh
